...
async function registerGameRoutes(app) {
    // Get game questions
    app.get('/games/:gameType/questions', { preHandler: auth_1.authenticateToken }, async (request, reply) => {
        try {
            const { gameType } = request.params;
            const questions = GAME_QUESTIONS[gameType] || [];
            if (questions.length === 0) {
                throw new helpers_1.ApiError(404, `Game type "${gameType}" not found`);
            }
            return reply.send({
                gameType,
                questions,
    ...
    // Submit game result
    app.post('/chat/sessions/:chatSessionId/game-result', { preHandler: auth_1.authenticateToken }, async (request, reply) => {
        try {
            const { chatSessionId } = request.params;
            const { gameType, result } = request.body;
