...
function referralExpiresAt(generatedAt: Date): Date {
...
export async function registerSubscriptionRoutes(app: FastifyInstance) {
  ...
  app.post(
    '/subscription/referral/regenerate',
    { preHandler: authenticateToken },
    async (request, reply) => {
      try {
        if (!request.userId) throw new ApiError(401, 'Unauthorized');
  ...
  app.post<{ Body: { referralCode: string } }>(
    '/subscription/referral/use',
    { preHandler: authenticateToken },
    async (request: FastifyRequest<{ Body: { referralCode: string } }>, reply) => {
      try {
        const { referralCode } = request.body;
        if (!referralCode) throw new ApiError(400, 'referralCode is required');

        const referrerSub = await prisma.subscription.findUnique({
          where: { referralCode },
        });

        if (!referrerSub) throw new ApiError(404, 'Invalid referral code');

        const now = new Date();
        const expiresAt = referralExpiresAt(referrerSub.referralGeneratedAt);
        if (now > expiresAt) throw new ApiError(410, 'Referral link has expired');
        if (referrerSub.referralCount >= MAX_REFERRALS) throw new ApiError(409, 'Referral limit reached');
